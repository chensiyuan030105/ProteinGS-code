...
def train(raw_config: str, args: list[str]) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    if not cfg.disable_checkpoint:
        mc = ModelCheckpoint(
            dirpath=cfg.output,       # Directory to save checkpoints
            filename="epoch_{epoch}",  # Filename pattern including the epoch number
            save_top_k=-1,            # Save all checkpoints, not just top-k
            save_last=True,          # Do not save an extra 'last' checkpoint
    ...
    loggers = []
    if wandb:
        wandb["name"] += "-" + time.strftime("%Y%m%d-%H%M%S")
    ...
    if (isinstance(devices, int) and devices > 1) or (
        ...
        print("cfg.find_unused_parameters =", cfg.find_unused_parameters)
        # strategy = DDPStrategy(static_graph=True, find_unused_parameters=cfg.find_unused_parameters)
        strategy = DDPStrategy(static_graph=False, find_unused_parameters=cfg.find_unused_parameters)
