...
class Potential(ABC):
    ...
    def compute_gradient(self, coords, feats, parameters):
        ...
        if index.max().item() > coords.shape[-2] - 1:
...
class VDWOverlapPotential(FlatBottomPotential, DistancePotential):
    def compute_args(self, feats, parameters):
        ...
        ######### crop connected_chain_index based on chain_sizes
        cci = feats["connected_chain_index"][0].to(atom_chain_id.device).long()
        if cci.numel() == 0:
            connected_chain_index = cci
        else:
            # Keep only edges whose endpoints are within [0, num_chains-1]
            in_range = (cci >= 0) & (cci < num_chains)      # shape (2, E) bool
            keep = in_range.all(dim=0)                       # shape (E,) bool
            cci = cci[:, keep]
            
            if cci.numel() == 0:
                connected_chain_index = cci
            else:
                has_atoms = chain_sizes > 0                      # shape (num_chains,)
                mask_has_atoms = has_atoms.index_select(0, cci[0]) & has_atoms.index_select(0, cci[1])   # (E,)
                connected_chain_index = cci[:, mask_has_atoms]
        ##########
...
class SymmetricChainCOMPotential(FlatBottomPotential, DistancePotential):
    def compute_args(self, feats, parameters):
        ...
        # (2, E) tensor of chain pairs
        pi = feats["symmetric_chain_index"][0].to(atom_chain_id.device).long()
        if pi.numel() == 0:
            pair_index = pi
        else:
            # 1) keep only endpoints in [0, num_chains-1]
            in_range = (pi >= 0) & (pi < num_chains)      # (2, E)
            keep = in_range.all(dim=0)                    # (E,)
            pi = pi[:, keep]

            if pi.numel() == 0:
                pair_index = pi
            else:
                # 2) (optional) drop pairs touching chains with no atoms
                has_atoms = chain_sizes > 0               # (num_chains,)
                mask_has_atoms = has_atoms.index_select(0, pi[0]) & has_atoms.index_select(0, pi[1])  # (E_valid,)
                pi = pi[:, mask_has_atoms]

                # 3) (optional) keep only multi-atom chains
                multi_atom = chain_sizes > 1
                mask_multi = multi_atom.index_select(0, pi[0]) & multi_atom.index_select(0, pi[1])
                pi = pi[:, mask_multi]

                # 4) (optional) drop self-pairs and duplicates
                non_self = pi[0] != pi[1]
                try:
                    assert non_self.all(), "Self-pairs should have been removed in data processing!"
                except:
                    import pdb; pdb.set_trace()
                
                # pi = pi[:, non_self]
                # if pi.numel():
                #     # canonicalize columns and unique
                #     a, b = pi.min(dim=0).values, pi.max(dim=0).values       # (E,)
                #     keys = a * num_chains + b
                #     uniq, idx = torch.unique(keys, return_index=True)
                #     pi = pi[:, idx]
...
def get_potentials_simple(steering_args, boltz2=False):
    potentials = []
    potentials.extend(
        [
            VDWOverlapPotential(
...
def get_potentials_pbd(steering_args, boltz2=False):
    potentials = []
    potentials.extend(
        [
            SymmetricChainCOMPotential(
