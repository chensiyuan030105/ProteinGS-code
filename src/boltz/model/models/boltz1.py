...
# for physical loss
from boltz.model.potentials.potentials import get_potentials_pbd # get_potentials #,  get_potentials_simple
import json
import copy
import os
...
# from boltz.model.models.gauss_seidel_fast_diff import *
...
class Boltz1(LightningModule):
    ...
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        physically_informed: bool = False,
        ...
        self.physically_informed = physically_informed
        ...
        if pretrained_path:
            print(" >>> load pretrained_path <<< ")
            # Load the checkpoint
            checkpoint = torch.load(pretrained_path, map_location="cpu", weights_only=False)
            # Load all parameters into the model
            # strict=True ensures that keys must match exactly
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            self.training and self.structure_prediction_training and self.enable_gradient
        ):
        ...
        print(f"encoder took {end - start:.2f} seconds")
        ...
        if self.training and self.structure_prediction_training:
            ...
            dict_out.update(self.structure_module.project_pbd(
                feats=feats,
                dict_out=dict_out, 
        ...
        print(f"sample took {end - start:.2f} seconds")
        if self.confidence_prediction:
            dict_out.update(
    ...
    def configure_optimizers(self):
        ...
        if self.structure_prediction_training:
            # Train all trainable parameters, except those with freeze_keywords
            parameters = [
                p for name, p in self.named_parameters()
                if p.requires_grad and not any(kw in name for kw in freeze_keywords)
            ]
        else:
            # Only train confidence_module + structure_module.out_token_feat_update
            parameters = [
                p for name, p in self.confidence_module.named_parameters()
                if p.requires_grad and not any(kw in name for kw in freeze_keywords)
            ] + [
                p for name, p in self.structure_module.out_token_feat_update.named_parameters()
                if p.requires_grad and not any(kw in name for kw in freeze_keywords)
            ]
        ...
        # Optionally attach LR scheduler
    ...
    def on_load_checkpoint(self, checkpoint: dict[str, Any]) -> None:
        if self.use_ema and "ema" in checkpoint:
            # Initialize EMA using only trainable parameters
            self.ema = ExponentialMovingAverage(
                parameters=[p for p in self.parameters() if p.requires_grad],
                decay=self.ema_decay
            )

            # Retrieve shadow weights from the checkpoint
            shadow_params_ckpt = checkpoint["ema"]["shadow_params"]

            # Filter EMA weights to include only those corresponding to current trainable parameters
            compatible_params = {
                k: v
                for k, v in shadow_params_ckpt.items()
                if k in dict(self.named_parameters()) and dict(self.named_parameters())[k].requires_grad
            }

            # Load EMA weights if compatible, otherwise skip
            if self.ema.compatible(compatible_params):
                self.ema.load_state_dict(
                    {**checkpoint["ema"], "shadow_params": compatible_params},
