...
from boltz.model.models.gauss_seidel_fast_diff_implicit_warp_cg_all import diff_gauss_seidel_solve_all
import warp as wp
import gc
...
class AtomDiffusion(Module):
    ...
    def sample_schedule(self, num_sampling_steps=None):
        ...
        if num_sampling_steps == 1:
            sigmas = torch.tensor([self.sigma_max], device=self.device, dtype=torch.float32)
            sigmas = sigmas * self.sigma_data
            sigmas = F.pad(sigmas, (0, 1), value=0.0)  # append 0
            return sigmas
    ...
    def project_pbd(
        ...
        ######## PBD
        potentials = get_potentials_pbd(steering_args, boltz2=False)
        
        atom_coords_key = "denoised_atom_coords" if is_training else "sample_atom_coords"
        
        # Load data from the first potential to get coords
        # assert dict_out["denoised_atom_coords"].shape[0] == 1
        
        for bid in range(dict_out[atom_coords_key].shape[0]):
            if verbose:
                print(f"=====BID: {bid}/{dict_out[atom_coords_key].shape[0]}")
            
            coords = dict_out[atom_coords_key][bid].clone()
            ...
            for pi_, potential in enumerate(potentials):
                ...
                parameters = potential.compute_parameters(1.0 - 49.0 / 50.0)
                
                index, args, com_args, ref_args, operator_args = potential.compute_args(feats_bid, parameters)
                k_vals, lower_bounds, upper_bounds = args
                
                if verbose:
                    with torch.no_grad():
                        energy_pi = potential.compute(
                            coords.unsqueeze(0),
                            feats_bid,
                            parameters,
                        ) 
                    print(f"    Initial energy: {energy_pi.mean().item():.4f}")
            ...
            coords_updated, _, _, _, coord_log_all_iter = diff_gauss_seidel_solve_all(
                coords,
            ...
            with torch.no_grad():
                # Outer loop to iterate over each element in coord_log_all_iter
                for idx, coords_iter in enumerate(coord_log_all_iter):
                    print(f"Processing coords_updated from iteration {idx + 1}...")  # Print the index of the current coords_updated being processed
                    
                    # Inner loop to iterate over each potential
                    for pi_, potential in enumerate(potentials):
                        potential_name = potential.__class__.__name__  # Get the name of the potential class
                        print(f"  {pi_}: {potential_name}")  # Print the potential index and name
                        
                        # Compute parameters for the potential
                        parameters = potential.compute_parameters(1.0 - 49.0 / 50.0)
                        
                        # Compute arguments required for the potential
                        index, args, com_args, ref_args, operator_args = potential.compute_args(feats_bid, parameters)
                        k_vals, lower_bounds, upper_bounds = args
                        
                        # Compute the energy for the current coords_updated
                        energy_pi = potential.compute(
                            coords_iter.unsqueeze(0),  # Add a batch dimension if required (unsqueeze(0))
                            feats_bid,
                            parameters,
                        )
                        print(f"   energy: {energy_pi.mean().item():.4f}")  # Print the mean energy of the current potential         
            
            dict_out[atom_coords_key][bid] = coords_updated.clone()
            ...
            if verbose:
                with torch.no_grad():
                    for pi_, potential in enumerate(potentials):
                        ...
                        energy_pi = potential.compute(
                            coords_updated.clone().detach().unsqueeze(0),
                            feats_bid,
                            parameters,
                        ) 
                        print(f"    After energy: {energy_pi.mean().item():.4f}")
    ...
    def noise_distribution(self, batch_size):
        ...
        noise = self.sigma_data * self.sigma_max * torch.ones((batch_size,), device=self.device)
        return noise
