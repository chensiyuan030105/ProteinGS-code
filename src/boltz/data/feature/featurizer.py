...
def process_residue_constraint_features(
    ...
    # Since the tokens are cropped during training, we need to find all the remaining atoms
    remaining_atom_idx = []
    for token in data.tokens:
        start = token["atom_idx"]
        end = token["atom_idx"] + token["atom_num"]
        remaining_atom_idx.extend(list(range(start, end)))
    ...
    if residue_constraints is not None:
        ...
        # -------------------- NEW: filter+remap to remaining_atom_idx --------------------
        # Build old->new compact index LUT (atoms not in 'remaining' map to -1)
        remaining_atom_idx_t = torch.tensor(remaining_atom_idx, dtype=torch.long)
        # Determine a safe LUT size from all indices that may appear
        def _safe_max(t: torch.Tensor) -> int:
            return int(t.max().item()) if t.numel() else -1
        lut_size = max(
            _safe_max(rdkit_bounds_index),
            _safe_max(chiral_atom_index),
            _safe_max(stereo_bond_index),
            _safe_max(planar_bond_index),
            _safe_max(planar_ring_5_index),
            _safe_max(planar_ring_6_index),
            (int(remaining_atom_idx_t.max().item()) if remaining_atom_idx_t.numel() else -1),
        ) + 1
        if lut_size < 1:
            # no valid atoms anywhere; drop everything to consistent empties below
            lut_size = 1
        old2new = torch.full((lut_size,), -1, dtype=torch.long)
        if remaining_atom_idx_t.numel():
            old2new[remaining_atom_idx_t] = torch.arange(remaining_atom_idx_t.numel(), dtype=torch.long)

        # Helper: remap & keep only columns whose all atoms survive
        def _remap_keep(idx: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
            # idx: (K, E)
            if idx.numel() == 0:
                return idx, torch.empty((0,), dtype=torch.bool)
            mapped = old2new.index_select(0, idx.reshape(-1)).reshape_as(idx)  # (K, E)
            keep = (mapped >= 0).all(dim=0)  # (E,)
            return mapped[:, keep], keep

        # RDKit bounds (2, E)
        rdkit_bounds_index, keep_rbc = _remap_keep(rdkit_bounds_index)
        if keep_rbc.numel():
            rdkit_bounds_bond_mask = rdkit_bounds_bond_mask[keep_rbc]
            rdkit_bounds_angle_mask = rdkit_bounds_angle_mask[keep_rbc]
            rdkit_upper_bounds = rdkit_upper_bounds[keep_rbc]
            rdkit_lower_bounds = rdkit_lower_bounds[keep_rbc]
        else:
        ...
        # Chiral atoms (4, E)
        chiral_atom_index, keep_ch = _remap_keep(chiral_atom_index)
        if keep_ch.numel():
            chiral_reference_mask = chiral_reference_mask[keep_ch]
            chiral_atom_orientations = chiral_atom_orientations[keep_ch]
        else:
            chiral_reference_mask = torch.empty((0,), dtype=torch.bool)
            chiral_atom_orientations = torch.empty((0,), dtype=torch.bool)

        # Stereo bonds (4, E)
        stereo_bond_index, keep_sb = _remap_keep(stereo_bond_index)
        if keep_sb.numel():
            stereo_reference_mask = stereo_reference_mask[keep_sb]
            stereo_bond_orientations = stereo_bond_orientations[keep_sb]
        else:
            stereo_reference_mask = torch.empty((0,), dtype=torch.bool)
            stereo_bond_orientations = torch.empty((0,), dtype=torch.bool)

        # Planar / ring indices (K, E) – just remap & drop
        planar_bond_index, _ = _remap_keep(planar_bond_index)
        planar_ring_5_index, _ = _remap_keep(planar_ring_5_index)
        planar_ring_6_index, _ = _remap_keep(planar_ring_6_index)
...
def process_chain_feature_constraints(
    ...
    # --- NEW: filter/remap connected_atom_index to atoms that remain after token cropping ---
    # Build remaining_atom_idx from tokens (same logic you use elsewhere)
    remaining_atom_idx: list[int] = []
    for token in data.tokens:
        start = token["atom_idx"]
        end = start + token["atom_num"]
        remaining_atom_idx.extend(range(start, end))
    remaining_atom_idx_t = torch.tensor(remaining_atom_idx, dtype=torch.long)

    if connected_atom_index.numel() > 0:
        # Create old->new compact index LUT; atoms not present map to -1
        max_conn = int(connected_atom_index.max().item()) if connected_atom_index.numel() else -1
        max_keep = int(remaining_atom_idx_t.max().item()) if remaining_atom_idx_t.numel() else -1
        lut_size = (max(max_conn, max_keep) + 1) if max(max_conn, max_keep) >= 0 else 0
        ...
        if lut_size > 0:
            ...
            mapped = old2new.index_select(0, connected_atom_index.reshape(-1)).reshape_as(connected_atom_index)  # (2, E)
            keep_atoms = (mapped >= 0).all(dim=0)  # keep only pairs whose both atoms survive
            connected_atom_index = mapped[:, keep_atoms]
        else:
            connected_atom_index = torch.empty((2, 0), dtype=torch.long)
    # ---------------------------------------------------------------------------------------
