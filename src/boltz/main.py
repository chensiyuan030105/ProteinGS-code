...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    max_parallel_samples: Optional[int] = 5,
    ...
    if model == "boltz2":
        ...
        gamma_0 = 0.8 if gamma_0 is None else gamma_0
        noise_scale = 1.003 if noise_scale is None else noise_scale
        ...
        gamma_0 = 0.605 if gamma_0 is None else gamma_0
        noise_scale = 0.901 if noise_scale is None else noise_scale
    ...
    if filtered_manifest.records:
        ...
        def merged_checkpoint_path(primary_ckpt: Path, aux_ckpt: Path) -> Path:
            merged_path = primary_ckpt.parent / f"merged_{primary_ckpt.stem}_{aux_ckpt.stem}.ckpt"
            if merged_path.exists():
                return merged_path
            
            prim = torch.load(str(primary_ckpt), map_location="cpu", weights_only=False)
            aux  = torch.load(str(aux_ckpt),    map_location="cpu", weights_only=False)

            prim_sd = prim["state_dict"] if isinstance(prim, dict) and "state_dict" in prim else prim
            aux_sd  = aux["state_dict"]  if isinstance(aux,  dict) and "state_dict"  in aux  else aux
            # Are *all* prim_sd keys present in aux_sd?
            missing = set(prim_sd) - set(aux_sd)
            assert not missing, f"Missing keys in aux_sd: {sorted(missing)[:20]} (+{max(0,len(missing)-20)} more)"

            # Start from aux, then overwrite with primary so your provided ckpt takes precedence
            merged_sd = dict(aux_sd)
            merged_sd.update(prim_sd)

            # Reuse primary's checkpoint structure if present
            merged_blob = prim if isinstance(prim, dict) else {"state_dict": merged_sd}
            merged_blob["state_dict"] = merged_sd

            torch.save(merged_blob, merged_path)
        ...
        if checkpoint is None:
            ...
            if model == "boltz2":
                checkpoint = merged_checkpoint_path(Path(checkpoint), cache / "boltz2_conf.ckpt")
            else:
                checkpoint = merged_checkpoint_path(Path(checkpoint), cache / "boltz1_conf.ckpt")
